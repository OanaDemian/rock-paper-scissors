...
export class RockPaperScissors {
  ...
  game(playerOneChoice, playerTwoChoice) {
    ...
    if (this.playerOne.name === undefined || this.playerTwo.name === undefined)
      throw new Error(`Players must register their names.`);
  ...
  #handlesErrors(playerOneChoice, playerTwoChoice) {
    if (!choices.includes(playerOneChoice))
      throw new Error(`${playerOneChoice} must be a valid choice.`);
    if (!choices.includes(playerTwoChoice))
      throw new Error(`${playerTwoChoice} must be a valid choice.`);
  }
